...
mat_version="_251031_1455";
myt1 = clock;
%% 1. 基础参数与路径配置
% 1.1 数据加载路径（PSO/IPSO优化结果文件，替换原GA/IGA数据）
% mat_name = 'ipso/dataset/best90_r100_Lenna.mat';  % 指定PSO/IPSO结果数据文件??????是否混合模糊，请查验
mat_name = "ipso\valid_dataset\best90_r100_100_2025-10-17.mat";
load(mat_name);  % 加载数据：含PSO_bestchrom、PSO_bestfitness、IPSO_bestchrom、IPSO_bestfitness等


% 1.2 实验控制参数（按老论文单图对比逻辑）
maxit = 1000;      % 图像还原对比次数（老论文常用1000次以统计稳定性）
...
%准备多图测试
% all_sim_picname = ["i01.tif", "i02.tif", "i03.tif", "i04.tif", "i05.tif", "i06.tif", "i07.tif", "i08.tif", "i09.tif", "i10.tif"];
all_sim_picname = strings(1,10);
%准备测试，方法1:读取img目录下所有的图片，自动加载到数组
     tifs = dir('ipso\valid\*.tif');
     % tifs = dir('ipso\*.tif');
     num_tifs = length( tifs );
     for i = 1 : num_tifs
    %    tif_file = fullfile( 'img\' , tifs(i).name );
       all_sim_picname(i) = tifs(i).name;
     end
% 方法2:读取指定数组
num_tifs = length( all_sim_picname );

% sim_picname="Barbara.tif";
all_sim_dir_picname = all_sim_picname;

[m,n] = size(all_sim_picname);

%初始化所有图片路径
for index=1:n
    str1 = all_sim_picname(index);
    % str2 = strcat("DIV2K_valid_HR_90_gray/", all_sim_picname(index));
    str2 = strcat("ipso\valid\", all_sim_picname(index));
    all_sim_dir_picname(index) = str2;
end
...
for pic_index = 1:n
    ...
    [P_Matrix, T_Matrix, image_resized, image_blurred] = Read_Pic_Add_Blurr1(sim_pic_dirname, picsize, inputnum);
...
%% 绘制三种算法最佳值对比图（带连线，600dpi + EPS格式，英文标签）
function plot_best_comparison(All_psnr_best, All_mse_best, All_ssim_best, algo_labels, image_names, save_path)
    ...
    %% 1. PSNR最佳值对比图
    hfig1 = figure('Position', [100, 300, 1200, 500], 'Visible', 'off'); % 增加宽度以容纳图例
    hold on;
    ...
    % 设置坐标轴
    xlim([0, num_images + 2]); % 右侧留白用于显示平均值
    ...
    % 在右侧显示平均值（无虚线）
    for algo = 1:3
        text(num_images + 1.2, psnr_means(algo), sprintf('Mean: %.2f', psnr_means(algo)), ...
    ...
    % 在右侧显示平均值（无虚线）
    for algo = 1:3
        text(num_images + 1.2, mse_means(algo), sprintf('Mean: %.2f', mse_means(algo)), ...
             'Color', colors(algo, :), 'FontSize', 10, 'FontWeight', 'bold', ...
             'HorizontalAlignment', 'left', 'FontName', 'Times New Roman', ...
             'BackgroundColor', 'white', 'EdgeColor', colors(algo, :), 'LineWidth', 1);
    end
    ...
    % 在右侧显示平均值（无虚线）
    for algo = 1:3
        text(num_images + 1.2, ssim_means(algo), sprintf('Mean: %.4f', ssim_means(algo)), ...
             'Color', colors(algo, :), 'FontSize', 10, 'FontWeight', 'bold', ...
             'HorizontalAlignment', 'left', 'FontName', 'Times New Roman', ...
             'BackgroundColor', 'white', 'EdgeColor', colors(algo, :), 'LineWidth', 1);
    end
...
% 
%     %% -------------------------- 1. PSNR对比图 --------------------------
%     hfig1 = figure('Visible','off', 'Position',[200 200 700 500]);  % 增加图形尺寸
%     h = bar(X, psnr_data, bar_width);
%     set(h, {'FaceColor'}, num2cell(colors, 2));
% 
%     % 动态调整Y轴范围
...
% 
%     % 图例放在右上角
%     hl = legend(data_rows, 'Location', 'northeast', ...
%                 'FontName', 'Times New Roman', 'FontSize', 10);
%     grid on;
% 
%     % 数值标注 - 改进版本
...
% 
%             % 根据数值位置决定标注位置
%             if val > (y_max - y_range * 0.15)
%                 text_pos_y = val - y_range * 0.04;  % 高数值放在柱子内部
%                 vert_align = 'top';
%             else
%                 text_pos_y = val + y_range * 0.02;  % 低数值放在柱子上方
%                 vert_align = 'bottom';
%             end
% 
%             text(text_pos_x, text_pos_y, sprintf('%.2f', val), ...
...
% 
%     %% -------------------------- 2. MSE对比图 --------------------------
%     hfig2 = figure('Visible','off', 'Position',[300, 200, 700, 500]);
%     h = bar(X, mse_data, bar_width);
%     set(h, {'FaceColor'}, num2cell(colors, 2));
% 
%     % 动态调整Y轴范围
...
% 
%             if val > (y_max - y_range * 0.15)
%                 text_pos_y = val - y_range * 0.04;
%                 vert_align = 'top';
%             else
%                 text_pos_y = val + y_range * 0.02;
%                 vert_align = 'bottom';
%             end
% 
%             text(text_pos_x, text_pos_y, sprintf('%.2f', val), ...
...
% 
%     %% -------------------------- 3. SSIM对比图 --------------------------
%     hfig3 = figure('Visible','off', 'Position',[400, 200, 700, 500]);
%     h = bar(X, ssim_data, bar_width);
%     set(h, {'FaceColor'}, num2cell(colors, 2));
% 
%     % 动态调整Y轴范围
...
% 
%     %% -------------------------- 1. PSNR对比图 --------------------------
%     % 1.1 显式创建图形窗口（无无效配置，确保句柄有效）
%     % hfig = figure('Position', [200, 200, 600, 400]);  % 仅保留位置配置
%     % PSNR图（修改后）
%     % hfig = figure('Position', [200, 200, 600, 400], ...
%     %           'CloseRequestFcn', @(src,evt) set(src, 'Visible', 'off'));
% 
%     hfig = figure('Visible','on', ...
%               'Position',[200 200 600 400]);   % 隐藏即可，其余默认
% 
% 
% 
...
% 
%     % 1.2 坐标轴与格式配置
%     ylim([min(psnr_data(:)) - 0.05, max(psnr_data(:)) + 0.1]);  % 适配数据范围
%     set(gca, 'XTick', X, 'XTickLabel', algo_labels, ...
%         'FontName', 'Times New Roman', 'FontSize', 11);
%     ylabel('PSNR (dB)', 'FontName', 'Times New Roman', 'FontSize', 12);
%     xlabel('Algorithm', 'FontName', 'Times New Roman', 'FontSize', 12);
%     title(sprintf('PSNR Comparison (%s.tif)', image_name_clean), ...
%           'FontSize', 13, 'FontWeight', 'bold');  % 标题含原始图像名
%     hl = legend(data_rows, 'Location', 'North', 'Orientation', 'horizontal', ...
%                 'FontName', 'Times New Roman', 'FontSize', 10);
%     grid on;
% 
%     % 1.3 数值标注（左对齐，基于柱子端点计算位置）
%     for i = 1:length(h)  % i=1(Mean)、i=2(Best)
%         xe = h(i).XEndPoints;  % 每个柱子的右侧端点坐标
%         for j = 1:numel(xe)
%             val = psnr_data(i, j);
%             % 计算左对齐位置：右侧端点 - 单个柱子宽度 + 微小偏移（避免超出）
%             % text_pos_x = xe(j) - single_w + 0.02;
%             text_pos_x = xe(j)- single_w/4;
%             text_pos_y = val + 0.01;  % 高于柱子顶部，避免遮挡
%             text(text_pos_x, text_pos_y, sprintf('%.2f', val), ...
...
%     end
% 
%     % 1.4 保存图像（确保路径有效，使用清理后的名称）
%     fname = fullfile(save_path_base, sprintf('PSNR_Comparison_%s.tif', image_name_clean));
%     print(hfig, fname, '-dtiff', '-r600');  % 移除-painters，使用默认渲染（兼容性更好）
%     % close(hfig);  % 强制关闭窗口，释放句柄资源
%     clear hfig;   % 清除句柄变量，避免残留
% 
%     %% -------------------------- 2. MSE对比图（同逻辑） --------------------------
%     % hfig = figure('Position', [300, 200, 600, 400]);
%     % MSE图（修改后）
%     % hfig = figure('Position', [300, 200, 600, 400], ...
%     %           'CloseRequestFcn', @(src,evt) set(src, 'Visible', 'off'));
% 
%         hfig = figure('Visible','off', ...
%               'Position',[300, 200, 600, 400]);   % 隐藏即可，其余默认
% 
% 
%     h = bar(X, mse_data, bar_width);
%     set(h, {'FaceColor'}, num2cell(colors, 2));
...
% 
%     %% -------------------------- 3. SSIM对比图（同逻辑） --------------------------
%     % hfig = figure('Position', [400, 200, 600, 400]);% SSIM图（修改后）
%     % hfig = figure('Position', [400, 200, 600, 400], ...
%     %           'CloseRequestFcn', @(src,evt) set(src, 'Visible', 'off'));
% 
%         hfig = figure('Visible','off', ...
%               'Position',[400, 200, 600, 400]);   % 隐藏即可，其余默认
% 
%     h = bar(X, ssim_data, bar_width);
%     set(h, {'FaceColor'}, num2cell(colors, 2));
% 
...
% function generate_comparison_figures(psnr_data, mse_data, ssim_data, algo_labels, data_rows, save_path_base, image_name)
% 
...
% 
%     % 基本重构
%     filename = ['PSNR_Comparison_' strrep(image_name, '.', '_') '.tif'];
%     full_path = fullfile(save_path_base, filename);
%     print(full_path, '-dtiff', '-r600');
