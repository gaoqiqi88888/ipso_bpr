...
mat_name =  'D:\matlab dev\ipso_bpr_v3\ipso\dataset\best90_0801_pop50_gen50_20260211.mat';
...
for pic_index = 1:n
    ...
    net = newff(inputn, outputn, hidden_layers);
    ...
    for index = 1:maxit
        ...
        [BPR_psnr(1,index), BPR_mse(1,index), ~, BPR_ssim(1,index), ...
         image_resized, image_blurred, BPR_restored] = Sim_Pic_PSO_L2(0, net, inputn, outputn, x, ...
         inputnum, hidden_layers, outputnum, sim_pic_dirname, picsize);
        ...
        [PSOBPR_psnr(1,index), PSOBPR_mse(1,index), ~, PSOBPR_ssim(1,index), ...
         image_resized, image_blurred, PSO_restored] = Sim_Pic_PSO_L2(0, net, inputn, outputn, x, ...
         inputnum, hidden_layers, outputnum, sim_pic_dirname, picsize);
        % 存储当前次复原图像
        ...
        [IPSOBPR_psnr(1,index), IPSOBPR_mse(1,index), ~, IPSOBPR_ssim(1,index), ...
         image_resized, image_blurred, IPSO_restored] = Sim_Pic_PSO_L2(0, net, inputn, outputn, x, ...
         inputnum, hidden_layers, outputnum, sim_pic_dirname, picsize);
        % 存储当前次复原图像
